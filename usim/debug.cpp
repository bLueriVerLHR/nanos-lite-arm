...
const char *Debug::REG_NAMES[] = {"r0",  "r1",  "r2",  "r3", "r4",  "r5",
                                  "r6",  "r7",  "r8",  "r9", "r10", "r11",
                                  "r12", "r13", "r14", "r15"};

const char Debug::FLAGS[] = {'c', 'v', 'n', 'z'};

Debug::Debug(uint32_t *regs, bool *c, bool *v, bool *n, bool *z,
             SystemBus *sysbus)
    : m_regs(regs), m_c(c), m_v(v), m_n(n), m_z(z), m_sysbus(sysbus) {
  /* pass */
}
...
uint32_t Debug::get_mem32(uint32_t addr) { return m_sysbus->read32(addr); }

void Debug::pause(uint32_t inst, const DecodedInst &di) {
  printf("0x%08x: %08x op=%02d imm=0x%08x", m_regs[15], inst, di.op, di.imm);
  std::cin.ignore(1);
}